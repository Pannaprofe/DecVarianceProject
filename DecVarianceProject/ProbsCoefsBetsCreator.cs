...
namespace DecVarianceProject
{
    ...
    public class ProbsCoefsBetsCreator
    {
        ...
        public ProbsCoefsBetsCreator()
        {
            ...
            _instance.ProbsMarathon = new List<List<double>>();
            _instance.ProbsOtherCo = new List<List<double>>();
            _instance.CoefsMarathon = new List<List<double>>();
            _instance.CoefsOtherCo = new List<List<double>>();
            
            _instance.ClientsBets = new List<BetInfo>();
            _instance.AllBetsForTable = new List<AllBetsTableContent>();
        ...
        protected void ObtainData()
        {
            GenProbsMarathon();
            GenProbsOtherCo();
            _instance.CoefsMarathon = GetCoefs(_instance.ProbsMarathon);
            _instance.CoefsOtherCo = GetCoefs(_instance.ProbsOtherCo);
            GenAllBetsOfAllPlayers();
        }

        private void CreateProbsAndCoefsStructure()
        {
            _instance.GennedParams = new List<GennedParamsTableContent>();
            for (var i = 0; i < _instance.ProbsMarathon.Count;i++ )
            {
                var row = new GennedParamsTableContent()
                {
                    MatchNum = i,
                    ProbabilityP1 = _instance.ProbsMarathon[i][1],
                    ProbabilityP2 = _instance.ProbsMarathon[i][2],
                    ProbabilityX = _instance.ProbsMarathon[i][0],
                    CoefP1 = _instance.CoefsMarathon[i][1],
                    CoefP2 = _instance.CoefsMarathon[i][2],
                    CoefX = _instance.CoefsMarathon[i][0]
                };
                _instance.GennedParams.Add(row);
            }
        }
        // *********************BEGIN GENERATORS*********************************************
        private void GenAllBetsOfAllPlayers()
        {
            for (var i = 0; i < _instance.BetsNum; i++)
            {
                ...
                _instance.ClientsBets.Add(onePlayerBet);
            }
        }
        private void GenProbsMarathon()
        {
            ...
            for (var i = 0; i < _instance.MatchesNum; i++)
            {
                ...
                _instance.ProbsMarathon.Add(matchParams);
            }
        }
        private void GenProbsOtherCo()
        {
            ...
            try
            {
                for (var i = 0; i < _instance.MatchesNum; i++)
                {
                    var x1 = _instance.ProbsMarathon[i][1] + Math.Round(random.NextDouble() * (2 * delta), 3) - delta;
                    var x2 = _instance.ProbsMarathon[i][2] + Math.Round(random.NextDouble() * (2 * delta), 3) - delta;
                    var x = _instance.ProbsMarathon[i][0] + random.NextDouble() * (2 * delta) - delta;

                    var matchParams = new List<double>(){x,x1, x2};
                    _instance.ProbsOtherCo.Add(matchParams);
                }
            }
        ...
        private BetInfo GenerateBet(int betNum)
        {
            ...
            for (var i = 0; i < chosenMatches.Count; i++)
            {
                ...
                {
                    case 0:
                        coef *= _instance.CoefsMarathon[chosenMatches[i]][1];
                        prob *= _instance.ProbsMarathon[chosenMatches[i]][1];
                        break;
                    case 1:
                        coef *= _instance.CoefsMarathon[chosenMatches[i]][0];
                        prob *= _instance.ProbsMarathon[chosenMatches[i]][0];
                        break;
                    case 2:
                        coef *= _instance.CoefsMarathon[chosenMatches[i]][2];
                        prob *= _instance.ProbsMarathon[chosenMatches[i]][2];
                        break;
                }
            }
            ...
            _instance.AllBetsForTable.Add(row);
            return betinfo;
        }
        //**********************END GENERATORS***********************************************
    }
